...
def load_model(pc_hdf):
    ...
    feature_medians = su.load_hdf(pc_hdf, "feature_medians", dtype=float)

    return feature_genes, feature_assays, feature_medians
...
if __name__=="__main__":
    ...
    model_genes, model_assays, model_medians = load_model(pc_hdf)
    ...
    model_medians = model_medians[model_col_idx]

    # Replace NaNs with training set medians
    for j in range(Z_test.shape[1]):
        Z_test[np.isnan(Z_test[:,j]), j] = model_medians[j]

    # Save the transformed data to HDF
    with h5py.File(out_hdf, "w", driver="core") as f:
        su.write_hdf(f, "X", Z_test.transpose())
