...
def plot_data(in_hdf, w=6, h=5, suptitle="Multiomic dataset", ylabel="Cancer types", group_label_size=6):
    ...
    with h5py.File(in_hdf, "r") as f:
        ...
        #axs[0][0].set_ylabel(f"{ylabel} ({M} samples)")
        axs[0][0].text(-3.0, M/2, f"{ylabel} ({M} samples)", rotation=90, size=10, horizontalalignment="right",
        ...
        for label, midpoint, c, in zip(unq_labels, midpoints, counts):
            ...
            axs[0][0].text(-1.0, midpoint, f"{label}", rotation=0.0, size=group_label_size, 
