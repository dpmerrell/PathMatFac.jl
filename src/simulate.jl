...
function simulate_data(pathway_sif_data, 
    ...
    assay_moment_decomp = decompose_all_data_signal(data_moments_vec,
                                                    model.matfac.feature_batch_ids,
                                                    model.matfac.feature_losses;
                                                    mu_snr=mu_snr,
                                                    delta_snr=delta_snr,
                                                    theta_snr=theta_snr,
    ...
    theta_moments_vec = []
    for a in enumerate(assay_moment_decomp)
    ...
    sim_params = simulate_params(model.matfac.X_reg, 
                                 model.matfac.Y_reg,
                                 model.matfac.row_batch_ids,
                                 model.matfac.col_batch_ids,
                                 logsigma_moments_vec,
                                 mu_moments_vec,
                                 logdelta_moments_vec,
                                 theta_moments_vec)

    D_gpu = BMF.simulate_data(sim_params, model.matfac.noise_models)
